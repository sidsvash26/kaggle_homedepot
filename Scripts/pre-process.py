# -*- coding: utf-8 -*-
"""
Created on Sat Apr 16 12:32:54 2016
...
#A function to make all lowercase
...
#A function that stems all the words in a string    using Snowbell       
def stem(string):
    words = word_tokenize(string)
    stem_sentence = []
    for word in words:
...
#Public dictionary for fixing typos on Kaggle, using google search
spell_check_dict={
...
def string_replace(s):
    if isinstance(s, str):
        #unit replacements        
        s = re.sub(r"([0-9]+)( *)(inches|inch|in|')\.?( )", r"\1 inch ", s)
        s = re.sub(r"([0-9]+)( *)(foot|feet|ft|'')\.?( )", r"\1 ft. ", s)
        s = re.sub(r"([0-9]+)( *)(pounds|pound|lbs|lb)\.?( )", r"\1 lb. ", s)
        s = re.sub(r"([0-9]+)( *)(square|sq)\.?( )", r"\1 sq. ", s)
        s = re.sub(r"([0-9]+)( *)(cubic|cu)\.?( )", r"\1 cu. ", s)
        ...
        s = re.sub(r"([0-9]+)( *)(volts|volt|v)\.?( )", r"\1 volt ", s)
...
#Apply Pre-processing to different datasets

#Correct public typos
df_all.replace({'search_term':spell_check_dict}, inplace=True)

#Product dataset
df_prod["product_description"] = df_prod["product_description"].map(lambda x: string_replace(x))
df_prod["product_description"] = df_prod["product_description"].map(lambda x: lower_case(x))
...
df_all["product_title"] = df_all["product_title"].map(lambda x: lower_case(x))
...
df_all.to_pickle('/home/sidvash/kaggle_2016/home_depot/dataframes/df_preProcess.pkl')        
